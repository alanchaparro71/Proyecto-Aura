...
function setMode(mode){
    ...
    // Guardar elección de emoción
    if(!emotionCount[mode]){
        emotionCount[mode] = 0;
    }
    ...
    switch(mode){
        ...
        case "misterioso":
            ...
            body.style.background =
            "linear-gradient(135deg,#1e1b4b,#000)";

        break;

    }

    mostrarEmocionFavorita();
...
let clicks = 0;
let seconds = 0;
let emotionCount = JSON.parse(
    localStorage.getItem("auraEmotions")
) || {};
...
function mostrarEmocionFavorita(){

    let favorita = "";
    let mayor = 0;


    for(let emocion in emotionCount){

        if(emotionCount[emocion] > mayor){

            mayor = emotionCount[emocion];
            favorita = emocion;
